...
- [x] 将上下文压缩升级为 cheap-first 四层策略：大结果缩减、低价值内容清理、旧工具结果占位、最后才调用模型摘要（失败时本地摘要兜底）
...
记忆快照在每个用户轮次开始时注入 system prompt，轮次内由工具写入的新记忆从下一用户轮次生效。记忆只被标记为参考上下文，不能覆盖系统安全规则。

## 上下文压缩与真实用量

当前 prompt 达到窗口 80% 时依次执行：

1. 缩减旧轮次中的超大工具结果，并保留可识别预览。
2. 移除旧 reasoning、工具调用旁白和临时阶段提示等低价值内容。
3. 把仍然过长的旧工具结果替换成带 call id 与原长度的占位符。
4. 若仍未降到 60% 目标，才额外调用模型生成延续摘要；调用失败或返回空内容时自动使用确定性本地摘要。
...
- 上下文达到配置窗口的 80% 时启动四层压缩，并以约 60% 为预算目标。
